/*
环境 VS2017 其他的未测试
C++ 可以理解成C的扩展和补充，C++兼容C；具体情况：https://www.cnblogs.com/ITziyuan/p/9487760.html
*/
#include <easyx.h>//C/C++的图形库
#include <conio.h>//conio是Console Input/Output（控制台输入输出）的简写，可理解为键盘的输入输出
#include <cstdio>//标准C的头文件
#include <ctime>//标准C的time头文件
#include <cmath>//标准C的math头文件
#pragma comment( lib, "MSIMG32.LIB")//C++引包的模式 MSIMG32.LIB VS自带的动态链接库，具体参照：https://baike.baidu.com/item/msimg32.dll/10959399?fr=aladdin
////////// 窗口大小 //////////
const short ScreenWidth = 640;//const 与C语言差不多 设置静态变量；具体可以参照：https://www.cnblogs.com/wkfvawl/p/10973578.html
const short ScreenHeight = 480;
////////// 全局变量 //////////
long ret = 0;
MOUSEMSG msg;//Easyx库中 鼠标的结构体
const short gap = 20;
const short xSide = ScreenWidth / 2 + gap;
const short ySide = ScreenHeight / 2 + gap;
IMAGE* imgMap = new IMAGE(ScreenWidth * 4, ScreenHeight * 4); //定义图像对象
...
public:
	...
	{
		Count++;
		isDead = false;//死亡状态 false 为活着，true 为死了
		length = 50 + rand() % 50;//蛇的长度
		nNode = length / 5;//蛇的节数
		maxNode = 9999;//最大节数
		imgHead = imgNode = imgTail = nullptr;// 蛇节点
		headNode = tailNode = nullptr;// 蛇节点
		nodeMsg = nullptr;//派生类指针 类型 POINT* nodeMsg 具体：https://zhidao.baidu.com/question/187475476.html
	}
	// 析构函数  C++特有的  可以理解为 构造函数为创造 析构函数为销毁 具体：https://www.cnblogs.com/tweechalice/p/11708579.html
	virtual ~SnakeBase()
	{
		Count--;
		Node* temp = headNode;
		...
		delete imgHead, imgNode, imgTail;//delete重定义，与Java有点像  void __CRTDECL operator delete
		if (nodeMsg != nullptr)
			delete[] nodeMsg;
	}
	// 设置image
...
int SnakeBase::Count = 0;//引用静态成员 关于C++ 的 :: 具体参照：https://blog.csdn.net/qq_21445563/article/details/80622108 
////////// Player 类 //////////
...
int main()
{
	initgraph(ScreenWidth, ScreenHeight);//初始化游戏地图
	srand((unsigned)time(NULL));//随机数
	Game game;//创建Game类的对象，与Java相比有所不同，具体参照：https://www.cnblogs.com/weekbo/p/8184141.html
	game.Running();//调用Game中的running函数
	settextcolor(BLACK);//EasyX库函数 settextcolor设置当前文字颜色
	settextstyle(50, 0, _T("微软雅黑"));//EasyX库函数 设置当前字体样式
	outtextxy((ScreenWidth - textwidth(_T("游戏结束"))) / 2, ScreenHeight / 3, _T("游戏结束"));//EasyX库函数 在指定位置输出字符串
	settextstyle(20, 0, _T("微软雅黑"));
	outtextxy((ScreenWidth - textwidth(_T("按任意键退出"))) / 2, ScreenHeight / 4 * 3 + 50, _T("按任意键退出"));
	FlushBatchDraw();//EasyX库函数 执行未完成的绘制任务
	while (_kbhit())//conio 中键盘持续获取输入的，具体参考：https://blog.csdn.net/qq_43604256/article/details/86523997
		ret = _getwch();
	ret = _getwch();
	closegraph();// 关闭图形环境
	return 0;
