...
#                                 'className': '111', 'healthCode': '111'}
                        ...
                        # 检测到人脸在人脸库中，返回数据，显示个人信息
                            ...
                            if float(dataSuccess['temperature']) <= 37.2 and dataSuccess['healthCode'] == '绿色':
                                ...
                                controlDoor('CLOSE')
                            # 体温小于37.2且健康码为黄色 不通行
                            elif float(dataSuccess['temperature']) <= 37.2 and dataSuccess['healthCode'] == '橙色':
                                pathStateSuccess = os.getcwd()+"/image/state07.png"
                                self.breakSignal2.emit(dataSuccess, pathFace, pathStateSuccess)
                            ...
                            # 体温小于37.2且健康码为红色 不通行
                                ...
                                pathStateSuccess = os.getcwd()+"/image/state08.png"
                                self.breakSignal2.emit(dataSuccess, pathFace, pathStateSuccess)
