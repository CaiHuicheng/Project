...
//RGB和depth生成云点
void createPCL(PointCloud::Ptr& cloud, FRAME& f);
//点云预处理
...
int main(int argc, char** argv)
{
	...
	string rgb_dir = "J:/JustIT-USY/CaiHuicheng-practice/OpenCV/3Dreconstruction/3Dreconstruction/data/bear/color/color_";
	//J:/JustIT-USY/CaiHuicheng-practice/OpenCV/3Dreconstruction/3Dreconstruction/data/output0/color
	string depth_dir = "J:/JustIT-USY/CaiHuicheng-practice/OpenCV/3Dreconstruction/3Dreconstruction/data/bear/depth/depth_";
	...
	//读取文件第一张图片
	FRAME frame1;
	frame1.rgb = imread("J:/JustIT-USY/CaiHuicheng-practice/OpenCV/3Dreconstruction/3Dreconstruction/data/bear/color/color_1.png");
	frame1.depth = imread("J:/JustIT-USY/CaiHuicheng-practice/OpenCV/3Dreconstruction/3Dreconstruction/data/bear/depth/depth_1.png", -1);
	//frame1.rgb = imread("color_0.png");
	//frame1.depth = imread("depth_0.png");
	cout << "\nprocessing the 1 images" << endl;
	createPCL(output, frame1);
	...
	*cloud1show += *output;*/
	...
	for (int i =2; i < 9; i++)
	{
		...
		cout << "SE(3)拼接..." << endl;
		output = joinPCL(output, frames[j + 1]);
...
PointCloud::Ptr joinPCL(PointCloud::Ptr& output, FRAME& frame, Mat& r, Mat& t)
{
	...
	pcl::transformPointCloud(*output, *output_, T.matrix());
	*cloud += *output_;
	cout << "正在拼接图像..." << endl;
	...
	pass.setFilterLimits(0.0, 0.5);//4m以上就舍弃
...
PointCloud::Ptr joinPCL(PointCloud::Ptr& output, FRAME& frame)
{
	...
	//参数设置，部分可缺省
	icp.setMaxCorrespondenceDistance(0.1);	//对应点间的最大距离(单位为m)
